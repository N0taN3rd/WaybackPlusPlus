  ...
  function inject () {
    ...
    class GenericIARewritter {
      ...
      static init () {
        ...
        if (match) {
          const rewriter = new GenericIARewritter(
            { match, isSave: false },
            true
          )
          rewriter.overrides()
      ...
      overrides () {
        ...
        if (window.Node) {
          ...
          window.Node.prototype.insertBefore = function insertBefore (
            newC,
            ref
          ) {
            if (newC) {
          ...
          window.Node.prototype.replaceChild = function replaceChild (
            newC,
            oldC
          ) {
            if (newC) {
        ...
        if (window.SVGImageElement && window.SVGImageElement.prototype) {
          ...
          const oGetAttributeNS =
            window.SVGImageElement.prototype.getAttributeNS
          ...
          const oSetAttributeNS =
            window.SVGImageElement.prototype.setAttributeNS
          ...
          if (string && string.indexOf('<html') >= 0) {
            ...
            if (needsEnd) {
              if (inner_doc.all.length > 3) {
                let end_tag =
                  '</' + inner_doc.all[3].tagName.toLowerCase() + '>'
                if (
          ...
          if (children) {
            for (let i = 0; i < children.length; ++i) {
              if (children[i].nodeType === Node.ELEMENT_NODE) {
                ...
                console.log(
                  'rewriteElementRecursive not considered',
                  children[i]
                )
              }
            }
      ...
      rewriteElement (elem) {
        ...
        if (lowerName === 'style') {
          ...
          for (const [propName, { getter, setter }] of Object.entries(
            toCheck
          )) {
            if (getter) {
...
class Handler {
  ...
  async doIt (url, odId) {
    if (!this.activeTabs.has(url)) {
      const newTab = await this.createNewTab()
      const ntInfo = { url, odId, newTab }
      this.activeTabs.set(newTab.id, ntInfo)
      this.activeTabs.set(url, ntInfo)
      this.activeTabs.set(odId, ntInfo)
      await this.attatchToTab(newTab.id)
      await this.sendCommand(newTab.id, 'Page.enable')
      let wasError = false
      try {
      ...
      if (wasError) {
        await this.sendCommand(newTab.id, 'Page.addScriptToEvaluateOnLoad', {
          scriptSource: injectString
        })
      }
      await this.updateTabNav(newTab.id, url)
    } else {
      await this.stop(url)
    }
  }

  async stop (whoToStop) {
    let ntInfo = this.activeTabs.get(whoToStop)
    await this.detachFromTab(ntInfo.newTab.id)
    if (localStorage.getItem('w++close-tab') === 'yes') {
      await this.removeTab(ntInfo.newTab.id)
      await this.setTabActive(ntInfo.odId)
    }
    this.activeTabs.delete(ntInfo.newTab.id)
    this.activeTabs.delete(ntInfo.url)
    this.activeTabs.delete(ntInfo.odId)
  }

  createNewTab () {
    return new Promise((resolve, reject) => {
      chrome.tabs.create({ active: true, url: 'about:blank' }, tab => {
        resolve(tab)
      })
    })
  }
  ...
  sendCommand (tabId, whichOne, args) {
    return new Promise((resolve, reject) => {
      chrome.debugger.sendCommand({ tabId }, whichOne, args, results => {
        if (chrome.runtime.lastError) {
          reject(new Error(chrome.runtime.lastError.message))
        } else {
          resolve(results)
...
chrome.tabs.onRemoved.addListener(async tabId => {
  if (handler.activeTabs.has(tabId)) {
    await handler.stop(tabId)
  }
})

chrome.debugger.onDetach.addListener(async (source, reason) => {
  if (source.tabId && handler.activeTabs.has(source.tabId)) {
    await handler.stop(source.tabId)
  }
})

const showRe = /https?:\/\/web\.archive\.org\/web\/[0-9]+\/*/
// const showRe = /https?:\/\/web\.archive\.org*/

chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.url && changeInfo.url.match(showRe)) {
    chrome.pageAction.show(tabId)
    if (chrome.runtime.lastError) {
    }
  } else if (tab.url && tab.url.match(showRe)) {
    chrome.pageAction.show(tabId)
    if (chrome.runtime.lastError) {
    }
  }
...
async function listenForPopupMsg (request, sender, sendResponse) {
  if (request.wbpp) {
    if (request.wbpp === 'isWorking') {
      sendResponse({
        isWorking: handler.activeTabs.has(request.tab.id),
        tab: request.tab
      })
    } else if (request.wbpp === 'start') {
      await handler.doIt(request.url, request.id)
    } else if (request.wbpp === 'stop') {
      await handler.stop(request.url)
    }
  }
}

chrome.runtime.onMessage.addListener(listenForPopupMsg)
